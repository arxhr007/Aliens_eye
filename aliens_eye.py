...
parser.add_argument(
    "username", nargs="*", help="- pass the username , example:  $aliens_eye aaron123"
)
...
def main(usernames):
    ...
    for username in usernames:
        ...
        print(f"{g}#" * 126)
        print(
            f"{g}# {r}SOCIAL MEDIA   {g}|        {r}USER {g}        | {r}STATUS CODE{g} | {r}                   URL   {g}      {spece}                   #"
        )
        thread1 = threading.Thread(
            target=scanner, args=(username, dict(itertools.islice(social.items(), 40)))
        )
        thread2 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 40, 80))),
        )
        thread3 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 80, 120))),
        )
        thread4 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 120, 160))),
        )
        thread5 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 160, 200))),
        )
        thread6 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 200, 240))),
        )
        thread7 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 240, 280))),
        )
        thread8 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 280, 320))),
        )
        thread9 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 320, 360))),
        )
        thread10 = threading.Thread(
            target=scanner,
            args=(username, dict(itertools.islice(social.items(), 360, 435))),
        )
        thread1.start()
        thread2.start()
        thread3.start()
